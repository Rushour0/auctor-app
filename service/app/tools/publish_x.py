...
async def _consume_approval(
    db: AsyncIOMotorDatabase, client_id: str, draft_id: str, approval_id: str
) -> str | None:
    """Atomically mark ``approval_id`` consumed for ``client_id`` and ``draft_id``.

    Returns None on success, or an error_message string if the approval doesn't exist, isn't for
    this client, isn't for this draft, isn't approved, or was already used. Scoped by client_id —
    fleet isolation.
    """
    ...
    existing = await db.approval_requests.find_one(
        {"approval_id": approval_id, "client_id": client_id}
    )
    if existing is None:
        return (
            f"missing_approval: no approval_id={approval_id!r} on file for client_id={client_id!r}."
        )
    if existing.get("artifact_id") != draft_id:
        return (
            f"missing_approval: approval_id={approval_id!r} was not issued for "
...
async def run(db: AsyncIOMotorDatabase, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        x_integration.require_app_credentials()
    ...
    # so a media_assets entry without a pre-resolved media_id fails loud instead of silently
    # publishing a text-only tweet.
    media_ids = [
        asset["media_id"]
        for asset in media_assets
        if isinstance(asset, dict) and asset.get("media_id")
    ]
    if media_assets and not media_ids:
        out["status"] = "failed"
        out["error_message"] = (
            "unsupported_media: media_assets requires a pre-resolved media_id; asset_url "
            "upload is not yet implemented."
        )
        return out

    approval_error = await _consume_approval(db, client_id, draft_id, approval_id)
