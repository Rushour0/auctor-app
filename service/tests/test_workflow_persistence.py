...
def test_run_observability_aggregates_agent_cost_tokens_and_latency() -> None:
    ...
    for event in (
        WorkflowEvent(
            ...
            idempotency_key="run-1:research:1",
            duration_ms=1200,
            input_tokens=100,
            output_tokens=40,
            cached_tokens=10,
        ...
        WorkflowEvent(
            ...
            parent_event_id="stage-1",
            agent="writer",
            stage="draft",
            outcome="failed",
            event_type="stage.failed",
            idempotency_key="run-1:draft:1",
            duration_ms=800,
            input_tokens=200,
            output_tokens=80,
            cost_usd=0.07,
    ...
    for run_id, outcome, cost, duration in (
        ...
        workflow.record_event(
            WorkflowEvent(
                ...
                stage="publish",
                outcome=outcome,
                event_type="run.completed" if outcome == "succeeded" else "run.failed",
...
def test_observed_stage_derives_duration_and_records_usage() -> None:
    ...
    workflow.start_stage(
        WorkflowEvent(
            ...
            pipeline="content_loop",
            run_id="run-timed",
            stage_run_id="stage-timed",
            agent="voice_writer",
            stage="draft",
            event_type="ignored",
            idempotency_key="run-timed:draft:started",
            started_at=started,
        )
    )
    result = workflow.complete_stage(
        WorkflowEvent(
            workspace_id="workspace-1",
            fleet_id="fleet-1",
            client_id="client-1",
            ...
            outcome="succeeded",
            event_type="stage.completed",
            idempotency_key="run-timed:draft:completed",
